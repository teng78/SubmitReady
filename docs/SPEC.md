...
契约为 `SafeExtractor.extract(zip_path: Path, run_id: UUID) -> Path`；工作目录为配置根目录下的 `run_id`，已存在即拒绝。压缩包大小由 API 和 extractor 双重检查。成员计数包含目录项，上限采用 `<=`。总量同时检查中央目录声明和实际写出字节，上限采用 `<=`。

归档路径先把 `\` 转为 `/`，移除一个目录项末尾 `/`，再分段；拒绝空/NUL、以 `/` 或 `//` 开头、`^[A-Za-z]:`、空段、`.`、`..`、Windows 设备名、任何段尾的点/空格。以 casefold 后的标准路径检测重复名；拒绝文件/目录前缀冲突。拒绝加密成员、Unix mode 标识的符号链接及其他非普通文件/目录。提取不保留权限、所有者、时间戳或可执行位。CRC/损坏读取统一为 `INVALID_ZIP`。

稳定错误码：`ZIP_TOO_LARGE`、`TOO_MANY_FILES`、`EXTRACTED_TOO_LARGE`、`UNSAFE_PATH`、`UNSUPPORTED_ZIP_ENTRY`、`DUPLICATE_ZIP_ENTRY`、`INVALID_ZIP`、`WORKSPACE_EXISTS`。extractor 负责失败时删除新建工作目录；API 负责临时上传清理；成功目录由 run 生命周期保留，删除历史时 repository/service 尽力清理，失败记录警告但不伪称成功清理。
...
```yaml
schema_version: 1
id: python-course-v1       # ^[a-z][a-z0-9-]{2,63}$
name: Python Course       # 1..100 chars
language: python           # python | c | cpp
required_files: [main.py]  # 0..100 POSIX glob strings
forbidden_paths: [.env, .git/**]
ignore_paths: [.venv/**, __pycache__/**]
limits:
  max_file_bytes: 1048576       # 1..104857600
  max_project_bytes: 10485760   # 1..104857600; >= file limit
  max_depth: 5                  # 1..20
commands:
  build: [python, -m, py_compile, main.py] # null or 1..32 args
  test: [python, -m, pytest, -q]
  timeout_seconds: 15           # 1..120
  network_disabled: true
sensitive_scan: true
scoring:
  - id: structure               # unique slug
    label: Required structure
    weight: 30                  # integer 0..100; total exactly 100
```

字段默认值：三个 path 列表为空；limits 为 1 MiB/10 MiB/5；build/test 为 null；timeout 15；network_disabled true；sensitive_scan true；scoring 为空（为空时不计算分数）。glob 必须是相对 POSIX 风格，拒绝反斜杠、绝对路径、盘符和 `..` 段。

规则来源的唯一权威是文件系统：仓库 `examples/rules/*.yaml` 为内置只读规则，应用数据目录 `rules/*.yaml` 为自定义规则；数据库不保存 RuleRecord。API 返回内容 SHA-256 作为 `revision`。上传 ID 与任何已有规则冲突返回 409，不覆盖；更新必须换新 ID。模板是上述 YAML 的可下载文本。

命令校验：首参数只允许 basename（不得含路径），按语言允许 `python/python3/py`、`pytest`（python），`gcc/clang/make/cmake/ctest/ninja`（c），`g++/clang++/make/cmake/ctest/ninja`（cpp）。全语言拒绝 shell 元字符 `;&|><`、换行/NUL、URL、绝对路径和 `..` 参数。Python 的 `-c`/`--command` 拒绝；`-m` 后仅允许 `py_compile`、`pytest`、`unittest`。拒绝网络、shell、提权、删除和包安装可执行文件。命令来自导入后不可变规则快照。
...
`RunRecord(id UUID, project_name, project_hash, rule_id, rule_name, rule_revision, status, stage, started_at, finished_at, duration_ms, workspace_path, report_json)`。规则由文件系统管理，不进入数据库。当前 SQLite 以单表 JSON 报告换取简单迁移；规则快照嵌入报告，历史不受规则更新影响。
